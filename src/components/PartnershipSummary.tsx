...
const PartnershipSummary = () => {
  ...
  const deliverables = [
    ...
    },
    {
      ...
      features: [
        ...
        { type: 'text', value: 'Quarter Page' },
        { type: 'text', value: '1/8 Page' }
      ]
    },
    {
  ...
  const renderFeatureCell = (feature: any, partnershipIndex: number) => {
    ...
    if (feature === true) {
      ...
      return (
        <div className={`text-center py-2 px-3 ${partnership.bgColor} ${partnership.textColor} rounded text-sm font-medium`}>
          {feature.value}
        </div>
      );
    }
    ...
    if (feature === true) {
      return (
        <div className="flex items-center gap-2">
          <div className={`w-6 h-6 ${partnership.bgColor} rounded-full flex items-center justify-center flex-shrink-0`}>
      ...
      return (
        <div className={`inline-flex py-1.5 px-3 ${partnership.bgColor} ${partnership.textColor} rounded text-sm font-medium`}>
          {feature.value}
        </div>
      );
    }
  ...
  return (
    <section id="partnership-packages" className="py-16 md:py-24 relative overflow-hidden">
      <div className="max-w-7xl mx-auto  relative z-10">
        ...
        <div className="lg:hidden">
          ...
          <div className="flex items-center justify-between mb-8">
            <button
              onClick={() => setActivePartnership((prev) => (prev - 1 + partnerships.length) % partnerships.length)}
            ...
            <div className="flex-1 mx-4">
              <div className={`${partnerships[activePartnership].bgColor} ${partnerships[activePartnership].textColor} py-4 px-6 rounded-lg text-center`}>
                <h3 className="text-lg font-bold">{partnerships[activePartnership].name}</h3>
              </div>
            </div>
            
            <button
              onClick={() => setActivePartnership((prev) => (prev + 1) % partnerships.length)}
              className="p-2 bg-gray-800 rounded-lg text-gray-400 hover:text-white transition-colors"
            >
              ...
              <button
                ...
                className={`h-2 rounded-full transition-all duration-300 ${
                  index === activePartnership 
                    ? `w-8 ${partnerships[index].bgColor}` 
                    : 'w-2 bg-gray-600'
                }`}
              />
            ))}
          ...
          <div className="space-y-4">
            {deliverables.map((deliverable, index) => (
              <div key={index} className="bg-gray-900 rounded-lg p-4 border border-gray-800">
                ...
                <div className="pl-2">
                  {renderMobileFeature(deliverable.features[activePartnership], activePartnership)}
                </div>
              </div>
            ))}
          </div>
              ...
              <div className="min-w-[900px]">
                ...
                <div className="space-y-2">
                  {deliverables.map((deliverable, rowIndex) => (
                    <div key={rowIndex} className="grid grid-cols-6 gap-3 items-center py-3 border-b border-gray-800 last:border-0">
                      <div className="text-gray-300 text-sm pr-4">
                        {deliverable.title}
                      </div>
                      {deliverable.features.map((feature, colIndex) => (
                        <div key={colIndex} className="flex justify-center items-center min-h-[50px]">
