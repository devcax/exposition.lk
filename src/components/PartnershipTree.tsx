...
import { Award, Gem, Star, Shield, Crown, ChevronRight, X, ArrowRight } from 'lucide-react';
...
const PartnershipCard: React.FC<PartnershipCardProps> = ({ partnership, onSelect, isSelected }) => (
  <div
    onClick={onSelect}
  ...
  >
    <div className="flex items-start justify-between mb-4">
      <div className="flex items-center gap-3">
        <div className={`p-3 rounded-lg bg-gradient-to-br ${partnership.color}`}>
          {React.cloneElement(partnership.icon, { className: "h-6 w-6 text-black" })}
        </div>
        <div>
          <h4 className="text-lg font-semibold text-white">{partnership.level}</h4>
          {partnership.investment && (
            <p className="text-[#e3c767] font-medium">{partnership.investment}</p>
          )}
        </div>
      </div>
      <ArrowRight className={`w-5 h-5 transition-colors ${isSelected ? 'text-[#e3c767]' : 'text-gray-600'}`} />
    </div>
    <p className="text-gray-400 text-sm line-clamp-2">{partnership.description}</p>
  </div>
);
...
const PartnershipTree: React.FC = () => {
  ...
  return (
    <section id="partnerships" className="py-24 bg-gradient-to-br from-gray-900 via-black to-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-gray-400 max-w-3xl mx-auto">
            Join our community of partners and create lasting impact through strategic collaboration
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1">
              ...
              {selectedPartnership ? (
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    <div className={`p-3 rounded-lg bg-gradient-to-br ${selectedPartnership.color}`}>
                      {React.cloneElement(selectedPartnership.icon, { className: "h-6 w-6 text-black" })}
                    </div>
                    <div>
                      <h4 className="text-lg font-semibold text-white">{selectedPartnership.level}</h4>
                      {selectedPartnership.investment && (
                        <p className="text-[#e3c767]">{selectedPartnership.investment}</p>
                      )}
                    </div>
                  </div>
                  <p className="text-gray-400 mb-4">{selectedPartnership.description}</p>
                  {selectedPartnership.benefits && (
                    <ul className="space-y-2">
                      {selectedPartnership.benefits.slice(0, 4).map((benefit, idx) => (
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500 mb-4">Select a partnership package to view details</p>
            ...
            <div className="mt-6 bg-gradient-to-r from-[#aa7d39]/10 to-[#e3c767]/10 rounded-xl border border-[#aa7d39]/30 p-6">
              <h4 className="text-lg font-semibold text-white mb-2">Need Custom Partnership?</h4>
              ...
              <div className="p-8">
                <div className="flex items-start justify-between mb-6">
                  <div className="flex items-center gap-4">
                    <div className={`p-4 rounded-xl bg-gradient-to-br ${selectedPartnership.color}`}>
                      {React.cloneElement(selectedPartnership.icon, { className: "h-8 w-8 text-black" })}
                    </div>
                    <div>
                      <h3 className="text-2xl font-bold text-white">{selectedPartnership.level}</h3>
                      {selectedPartnership.investment && (
                        <p className="text-[#e3c767] text-lg">{selectedPartnership.investment}</p>
                      )}
                    </div>
                  </div>
                  <button
                ...
                <p className="text-gray-300 mb-6">{selectedPartnership.description}</p>
                
                {selectedPartnership.benefits && (
                  <div>
                    <h4 className="text-lg font-semibold text-white mb-4">Included Benefits</h4>
